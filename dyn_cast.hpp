    ...
    if (Equal(typeinfo_from, typeinfo_to)) {
        ...
        for (int i = 0; i < ti_fr->__base_count; ++i) {
            ...
            if (bcti->__offset_flags & abi::__base_class_type_info::__virtual_mask) {
                ptrdiff_t offset_offset = bcti->__offset_flags >> 8; // bytes. in most derived vtable
                const void* offset_ptr = static_cast<const char*>(GetVtablePtr(GetMostDerived(obj))) + offset_offset;
                ptrdiff_t offset = *static_cast<const ptrdiff_t*>(offset_ptr); // bytes. in most derived
                base_obj = static_cast<const char*>(GetMostDerived(obj)) + offset;
            } else {
